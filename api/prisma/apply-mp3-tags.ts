...
function splitArtistNames(artistStr?: string | null): string[] {
...
async function main() {
  ...
  for (const filename of files) {
    ...
    // 1) Tạo / lấy từng Artist theo name (findFirst + create, KHÔNG upsert)
    ...
    // 3) Tạo / cập nhật Track theo audioUrl (findFirst + update/create)
    ...
    // 4) Đảm bảo quan hệ many–to–many trong TrackArtist (KHÔNG upsert)
    for (let i = 0; i < artistRecords.length; i++) {
      const ar = artistRecords[i];

      const existingLink = await prisma.trackArtist.findFirst({
        where: {
          trackId: track.id,
          artistId: ar.id,
        },
      ...
      if (!existingLink) {
        ...
        console.log(
          `      🎼 Quan hệ track-artist đã tồn tại: ${ar.name}`,
        );
      }
    }
  }
