...
async function main() {
  ...
  if (artists.length) {
    const docs = artists.map((a: any) => {
      ...
      if (a.userId !== null) doc.userId = a.userId;
      return doc;
    });

    await db.collection("Artist").insertMany(docs);
  }
  ...
  if (playlistTracks.length) {
    await db.collection("PlaylistTrack").insertMany(
      playlistTracks.map((pt: any) => ({
        _id: `${pt.playlistId}_${pt.trackId}`,
